...
<script type="text/javascript">  
	...
	function cheakAll() {
		...
		if ($("#employeeCommandSpan").html().trim().length > 0) {
			return false;
		}
		return true;
	} 

	function employeeCommandFunction() {
		if ($("#employeeCommand").val().length>0) {
			$("#employeeCommandSpan").html("");
			$("#employeeCommandSpan").css("color", 'green');
			$.ajax({
		    	url:"queryemployeeCommand.action",
		    	type:"post",
		    	dataType:"text",
		    	data:"employeeCommand="+$("#employeeCommand").val(),
		    	success:function(result){
		    		if ("false"==result) {
		    			$("#employeeCommandSpan").html("员工命令错误");
		    			$("#employeeCommandSpan").css("color", '#B52726');
		    			return false;
		    		}
		    	}
		    });
		} else {
			$("#employeeCommandSpan").html("员工命令不能为空");
			$("#employeeCommandSpan").css("color", '#B52726');
			return false;
		}
	}
		...
		<form action="register.action" method="post" onsubmit="return cheakAll()">
			...
			<table class="tableSet">
				...
				<tr>
					...
					<td style="text-align:right;display:none;">是否员工：</td>
					<td style="display: none;"><input type="text" name="user.type" id="type" value="3"/> 
				</tr>
				<tr>
					<td style="text-align:right;">员工指令：</td>
					<td><input type="text" name="employeeCommand" id="employeeCommand" value="" onblur="employeeCommandFunction()"/> 
					<span style="color: red;">*</span> <span
						id="employeeCommandSpan"></span></td>
				</tr>
			</table>
			</div>
